...
from NashOrPass.limit_holdem.agents.mcq import MCQAgent
...
def extract_transitions_from_trajectory(trajectory):
    ...
    For MCQ learning, rewards are assigned later after the hand ends.
    """
    transitions = []
...
def assign_final_payoff_to_all_transitions(transitions, final_payoff):
    """
    Monte Carlo target assignment.

    Every decision made by the hero in this hand is trained toward the realized
    final payoff from the hand.

    This means train_step() should use:
    ...
    for t in transitions:
        t["reward"] = float(final_payoff)

        # Important:
        # We mark every transition as terminal from the learner's perspective
        # because MCQ does not bootstrap from next_obs.
        t["done"] = True
...
class TrainingMCQWrapper:
    """
    RLCard-compatible wrapper that lets us use epsilon during training.

    The agent itself stays greedy by default in eval_step().
    """
...
def main():
    ...
    # gamma is not used by pure MCQ train_step if target = reward.
    # Keeping it here is fine for experiment naming and later variants.
    ...
    run_name = (
        f"mcq_limit_holdem_vs_{args.opponent}"
        f"_seed{args.seed}"
        f"_gamma{args.gamma}"
        f"_lr{args.lr}"
    ...
    save_dir = Path("checkpoints/limit_holdem/mcq") / run_name
    metrics_dir = Path("results/limit_holdem/mcq/metrics") / f"gamma{args.gamma}"
    ...
    for episode in trange(1, args.episodes + 1):
        ...
        env.set_agents(
            [
                TrainingMCQWrapper(agent, epsilon=epsilon),
                opponent,
            ]
        )
        ...
        # MCQ change:
        # Instead of only assigning payoff to the last transition,
        # assign the realized final payoff to every hero transition.
        if transitions:
            final_payoff = float(payoffs[0])
            transitions = assign_final_payoff_to_all_transitions(
                transitions,
                final_payoff,
            )
        ...
        if episode % args.eval_every == 0:
            ...
            row = {
                ...
                # MCQ-specific note.
                "target_type": "monte_carlo_final_payoff",
